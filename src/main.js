...
document.addEventListener('DOMContentLoaded', function () {
		...
		*/
		...
		function loadFitFile(blob) {
			...
			fitParser.parse(blob, function (error, data) {
				if (error) {
					...
					if (document.getElementById('xaxis').options.length == 0) {
						...
						if ("laps" in data && data.laps.length > 0) xaxisObj.lap_number = "laps";
						for (var [ykey, yrecords] of Object.entries(xaxisObj)) {
							//console.log(ykey);
			...
			// update file droplist
			fitdataObj[filename] = {data: fitdata, local_timestamp: local_timestamp};
			var filenameSport = filename.slice(0,10) + "-" + fitdata.sessions[0].sport; 
			var filesOptions=document.getElementById("files");
			filesOptions.add(new Option(filenameSport, filename));
		...
		document.getElementById('cleanMap').onclick = function (e) {
			if (withGPS) {
				for (var k = 0; k < Ltracks.length; k++) {
					mymap.removeLayer(Ltracks[k]);
				}
				Ltracks = Ltracks.slice(-1);
				for (var k = 0; k < Lpopups.length; k++) {
		...
		document.getElementById('files').onchange = function (e) {
			...
			local_timestamp = fitdataObj[filename].local_timestamp;
			document.getElementById('ylist').value = null;
		}
